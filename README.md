...
| **Greenco Invoicing** | ✅ Built | Commission invoices are pushed to `invoices.greenco.co.uk` (`POST /api/external/invoices`) so they get a real invoice number, a PDF, an email and the overdue/chase flow. Manchester work is raised by Greenco Group Ltd and Liverpool work by Greenco Liverpool Ltd, each its own company there — the office is worked out from the site address on the contractor's invoice. Set `INVOICING_API_URL` / `INVOICING_API_KEY` / `INVOICING_COMPANY_ID_MANCHESTER` / `INVOICING_COMPANY_ID_LIVERPOOL`; the key must match `INTEGRATION_SECRET` there. Without it the commission module works standalone. |
